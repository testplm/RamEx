...
#' 
#' @export
draw.global.IRCA <- function(dataset, group) {
  outliers <- outliers_maha_chisquare(dataset)
  if (length(outliers) != 0) {
    dataset <- dataset[-outliers,]
  }
  cor_list <- Hmisc::rcorr(dataset, type = 'pearson')
...
#' 
#' @export
draw.local.IRCA <- function(dataset, bands_ann) {
  outliers <- outliers_maha_chisquare(dataset)
  if (length(outliers) != 0) {
    dataset <- dataset[-outliers,]
  }
...
global_IRCA <- function(object) {
  ...
  IRCA.interests <- lapply(unique(object@meta.data$group), function(x) {
    ...
    interests <- draw.global.IRCA(temp_data, x)
    dev.off()
    return(interests)
  })
...
#' 
#' @export
local_IRCA <- function(object, bands_ann) {
  dataset <- get.nearest.dataset(object)
  ...
  lapply(unique(object@meta.data$group), function(x) {
    ...
    draw.local.IRCA(temp_data, bands_ann)
    grDevices::dev.off()
  })
}
...
#' outliers <- outliers_maha_chisquare(data, p = 0.05)
#' print(outliers)
...
#' correlation <- rcorr_df(df)
#' print(correlation)
rcorr_df <- function(df) {
  df_mat <- proxy::as.matrix(df)
...
#' @examples
#' dm <- matrix(runif(25), ncol = 5)
#' vectorized_dm <- vectorize_dm(dm, group = c("A", "A", "B", "B", "C"), duplicate = FALSE)
#' print(vectorized_dm)
vectorize_dm <- function(dm, group = NULL, duplicate = TRUE) {
  ...
  if (!is.null(group)) {
    ...
    if (duplicate) {
      melt_dm <- subset(reshape2::melt(dm))
      melt_dm$value <- as.numeric(melt_dm$value)
    } else {
      dm[lower.tri(dm)] <- NA
      diag(dm) <- NA
      melt_dm <- subset(reshape2::melt(dm), !is.na(value))
      melt_dm$value <- as.numeric(melt_dm$value)
    }
    ...
    if (duplicate) {
      dm_value <- subset(reshape2::melt(dm))
      dm_value$value <- as.numeric(dm_value$value)
    } else {
      dm[lower.tri(dm)] <- NA
      diag(dm) <- NA
      dm_value <- subset(reshape2::melt(dm), !is.na(value))
      dm_value$value <- as.numeric(dm_value$value)
    }
...
#' @examples
#' dm <- rcorr(matrix(runif(25), ncol = 5))
#' vectorized_dm_rcorr <- vectorize_dm_rcorr(dm, group = c("A", "A", "B", "B", "C"), duplicate = FALSE)
#' print(vectorized_dm_rcorr)
...
#' @importFrom circlize circos.clear circos.par circos.initialize circos.trackPlotRegion circos.trackLines circos.text circos.lines circos.axis circos.link
#'
#' @examples
#' # Assume Corr_mat is a correlation matrix
#' Plot_global_chordDiagram_rpvalue(Corr_mat)
#'
