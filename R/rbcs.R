...
#'
#' @examples
#' RBCS_RF(object, outfolder = "RF_imps", threshold = 0.002, ntree = 1000)
RBCS_RF <- function(
    object,
    outfolder = "RF_imps",
    draw = TRUE,
...
#' Title
#'
#' @param result The result object containing the predictions and probabilities. It should have at least 'y' (predicted labels) and 'probabilities' (predicted probabilities).
#' @param filename The name of the file to save the mislabeling results. Defaults to 'mislabeling.xls'.
#' @param outdir The directory where the output file will be saved.
#'
#' @return None. This function writes the mislabeling results to a file specified by 'filename' in the 'outdir' directory.
#' @export
#'
#' @examples
#' save.rf.results.mislabeling(result, 'mislabeling_results.xls', '/path/to/output/directory')
